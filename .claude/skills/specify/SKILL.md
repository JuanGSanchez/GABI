...
- **Functional requirements** — numbered list (FR-1, FR-2, …). Each requirement is one verifiable user-visible behaviour. No stack words (no "Java", "Spring", "LibraryService", "@McpTool", "Derby", "pom.xml"), no method names.
...
Scan the draft for architecture choices, class names, library names, SQL or Spring annotation details, or API shapes. If found, rephrase in behaviour terms or move to a non-binding "Implementation Notes" section clearly marked as non-normative. Do not remove rationale; rephrase.
...
## Sources
- User requirement: SDD pipeline stage-1 skill (specify) for GABI Group E.
