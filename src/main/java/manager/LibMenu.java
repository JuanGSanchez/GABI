...
public final class LibMenu {
    ...
    public static void main(String[] args) {
        ...
        if (configProps.getProperty("database-isbuilt").equals("false")) {
            ...
            // Write-back fix (D-2): instead of writing to the CWD-relative
            // "src/utils/configuration.properties" (which breaks inside a fat-jar),
            // persist the updated flag to an external writable location under the
            // user's home directory. This keeps the classpath resource read-only and
            // makes the packaged artifact work without a writable install directory.
            ...
            try {
                ...
                try (OutputStream os = Files.newOutputStream(externalConfig)) {
                    configProps.store(os, rb.getString("program-properties"));
                }
        ...
        do {
            ...
            if (args.length != 0) {
                if (args.length == 2) {
                    try {
                        UserDerby.getInstance().tryDB(args[0], args[1].toCharArray(), rb);
...
//                Arrays.fill(password, 'x');
                ...
                UserDerby.getInstance().tryDB(name, password.toCharArray(), rb);
                isUser = true;
...
//                System.err.println("\n" + ife.getMessage() + "\n");
    ...
    private static void selectionBlock(User currentUser) {
        ...
        do {
            count = LibDBBook.getInstance().countDB(currentUser, rb);
            if (count != null) {
                nBooks = count[0];
                idBooks = count[1];
            }
            count = LibDBMember.getInstance().countDB(currentUser, rb);
            if (count != null) {
                nMembers = count[0];
                idMembers = count[1];
            }
            count = LibDBLoan.getInstance().countDB(currentUser, rb);
            if (count != null) {
                nLoans = count[0];
                idLoans = count[1];
            }
